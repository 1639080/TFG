                ...
                if trk["tracker"].time_since_update < 1 and len(trk["hands"]):
                    ...
                    if trk["hands"].action is not None:
                        ...
                        if action in [Event.SWIPE_LEFT, Event.SWIPE_LEFT3]:
                            ...
                            gesture_mapper.handle_gesture("DOUBLE_TAP")
                            print("Gesture detected: DOUBLE_TAP")
...
def run(args):
    ...
    # Run gesture system in background thread. because not possible the heart model to be background
