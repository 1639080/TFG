...
class GestureActionMapper:
    ...
    def handle_gesture(self, gesture_name):
        if gesture_name == "LITTLE_FINGER": #little finger
            ...
            self.graphics_engine.set_min_heartbeat() #min heartbeat (file main)
        elif gesture_name == "DROP":
            self.graphics_engine.set_max_heartbeat() #max heartbeat (file main)
        #elif gesture_name == "STOP":
            #self.graphics_engine.stop_heartbeat()
            #SPACE KEY eliminate. file main. NEED TO IMPLEMENT "STOP" HANDLING IN GESTURES
