...
#Main app for everything: rendering/input/scene
class GraphicsEngine:
    def __init__(self, models_data = [((0, -2, -10), (0, 0, 0), (1, 1, 1), 100 , [1]*100)], win_size=(1000, 800)): #1000, 800
        # init pygame modules
        pg.init()
        pg.mixer.init()
        # window size
    ...
    #carrega el frame a la GPU com a texture
        ...
        #carrega la webcam frame com texture
        self.webcam_texture.write(frame.tobytes())
        self.latest_camera_frame_ready = False

    #keyboard events and exit
    #eliminar els de j,k, l després tot va bé
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT or (event.type == pg.KEYDOWN and event.key == pg.K_ESCAPE):
                ...
                if event.key == pg.K_j:
                    ...
                    for obj in self.scene.objects:
                        if hasattr(obj, 'animate_heartbeat') and hasattr(obj, 'ppm'):
                            if obj.animate_heartbeat:
                                obj.animate_heartbeat = False
                                print("Heartbeat paused")
                            else:
                                obj.ppm = 100
                                obj.animate_heartbeat = True
                                print("Heartbeat resumed at normal PPM (150)")
                elif event.key == pg.K_x:
                    self.toggle_perspective_view()
    ...
    def stop_heartbeat(self): #space key to stop gesture. Now STOP gesture. Stop the beating entirely.
        for obj in self.scene.objects:
