...
All Kalman filters operate with a predict->update cycle. 

The predict step, predict(), uses the state transition matrix F to predict the state in the 
next time period. The state is stored as a gaussian (x, P), where
x is the state  vector, and P is its covariance. 
Covariance matrix Q specifies the process covariance. This prediction is called the *prior*, the estimate prior to incorporating the measurement.

The update step, update(), incorporates the measurement z with covariance R, into the state
estimate (x, P). The class stores the system uncertainty in S,
the innovation (residual between prediction and measurement in
measurement space) in y, and the Kalman gain in k. This computes
